...
int	ibc_api_init(unsigned ibas_ip4addr, unsigned short ibas_port, unsigned short my_port, int *async_signalled)
 { struct sockaddr_in	sin;


   if (IBC_ASSUMED_PACKET_SIZE != sizeof(IBC_DL))  return(IBC_BAD_ARCH);
...
int	ibc_api_set_mode(int mode)
 { int	r;
   ...
   if (-1 == ibc_api_rxtx_sock)  return(IBC_NOT_INIT);
   r = fcntl(ibc_api_rxtx_sock, F_GETFL);
   if (r < 0)  return(IBC_FCNTL_FAILED);
   if (IBC_NONBLOCK_MODE == mode)  { r |= O_NONBLOCK; }
   else  { r &= ~ O_NONBLOCK; }
   if (-1 == fcntl(ibc_api_rxtx_sock, F_SETFL, r))  return(IBC_FCNTL_FAILED);
   return(IBC_OK);
 }
...
int	ibc_api_listen(IBC_DL *dl)
 { int			r, sr, peerlen, i, found;
   ...
   if (NULL == dl) return(IBC_NUL_PTR);

   for (;;)
    {
      peerlen = sizeof(peeraddr);
      memset(&peeraddr, 0, sizeof(peeraddr));
      r = recvfrom(ibc_api_rxtx_sock, dl, sizeof(*dl), 0, (struct sockaddr *)(&peeraddr), &peerlen);
      sr = errno;
      if (-1 == r)					/* if interupted */
        { if (NULL != ibc_api_signalled) if (*ibc_api_signalled)
      ...
      if (r != sizeof(*dl)) continue;			/* foolproof checks */

      if (ibc_api_ibas_ip4addr != ntohl(peeraddr.sin_addr.s_addr))  continue;
      if (ibc_api_ibas_port != ntohs(peeraddr.sin_port))  continue;
      
      dl_copy = *dl;					/* make a copy for possible resent */
      r = ibc_ntoh_dl(dl);				/* convert to host byte order */
      if (IBC_OK != r)  continue;			/* CRC error (most likely) */
      if (IBAS_ALERT_ID != dl->ID) continue;		/* are we getting garbage or genuine alert ? */
      ...
      sendto(ibc_api_rxtx_sock, &dl_copy, sizeof(dl_copy), 0, (struct sockaddr *)(&peeraddr), sizeof(peeraddr));
      memcpy(&(ibc_api_dl_prev[0]), &(ibc_api_dl_prev[1]), sizeof(IBC_DL) * (IBC_API_DL_PREV_MAX - 1));
...
int	ibc_api_send_ping(int seqnum)
 { IBC_DL		dl;
   ...
   if (IBC_OK != (r = ibc_hton_dl(&dl)))  return(r);
   ...
   if (sizeof(dl) != sendto(ibc_api_rxtx_sock, &dl, sizeof(dl), 0, (struct sockaddr *)(&to_addr), 
				sizeof(to_addr)))
     {
...
int	ibc_api_dump_ping_reply(IBC_DL *dl, unsigned ip4addr, unsigned short port)
 { double		tsend, tresp;
   struct timeval	mytv;

   if (NULL == dl)  return(IBC_NUL_PTR);
   if (0 == (IBAS_ALERT_TEST_FLAG_R_PING & dl->a.test_flag))  return(IBC_BAD_ARG);

   tsend = dl->a.det_flags + (dl->a.att_flags) / 1000000.0;
   gettimeofday(&mytv, NULL);
   tresp = mytv.tv_sec + (mytv.tv_usec) / 1000000.0;

   printf("PING_REPLY: %d octets from %d.%d.%d.%d/%d: time=%.3f ms seqnum=%d\n",
		sizeof(*dl), 
		(int)((ip4addr >> 24) & 0xff), (int)((ip4addr >> 16) & 0xff),
		(int)((ip4addr >> 8) & 0xff), (int)(ip4addr & 0xFF),
		(int)port, (tresp - tsend) * 1000.0, dl->seqnum);
...
int	ibc_api_dump_alert(IBC_DL *dl, unsigned ip4addr, unsigned short port)
 { char		system_str_utc[99], buftime[IBAS_ALERT_TIME_SIZE + 1];
   ...
   if (NULL == dl)  return(IBC_NUL_PTR);
   if (IBAS_ALERT_TEST_FLAG_R_PING & dl->a.test_flag)  return(IBC_BAD_ARG);

   system_str_utc[0] = 0;
   system_tai = 0.0;
   if (IBC_OK == ibc_current_utc(&system_utc))
     { if (IBC_OK != ibc_utc2str24(system_utc, &(system_str_utc[0])))  { system_str_utc[0] = 0; }
       else  { system_str_utc[24] = 0; }
       if (IBC_OK != ibc_utc2tai2ert(&system_utc, &system_tai, NULL))  system_tai = 0.0;
     }

   alert_tai = 0.0;
   if (IBC_OK == ibc_str242utc(dl->a.pkt_time, &alert_utc))
     { if (IBC_OK != ibc_utc2tai2ert(&alert_utc, &alert_tai, NULL))  alert_tai = 0.0;
     }

   grb_tai = 0.0;
   if (IBC_OK == ibc_str242utc(dl->a.grb_time, &grb_utc))
     { if (IBC_OK != ibc_utc2tai2ert(&grb_utc, &grb_tai, NULL))  grb_tai = 0.0;
     }
   ...
   printf("dl.a.object_type           = %d\n", dl->a.object_type);

   printf("dl.a.crc16                 = %02X%02X\n", dl->a.crc16[0], dl->a.crc16[1]);

   printf("\n");
   printf("Reception time             : %s\n", system_str_utc);
   printf("(Reception - transmission) : %.4f [seconds] (assuming clocks synced)\n", system_tai - alert_tai);
   printf("(Reception - GRB event)    : %.4f [seconds] (assuming clocks synced)\n", system_tai - grb_tai);
   printf("\n");
