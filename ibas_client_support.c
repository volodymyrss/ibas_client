...
ushort2	ibc_crc_16(ushort2 crc, byte *p, int nbytes)
 {
   for (; nbytes > 0; nbytes--)  crc = (crc << BYTE_BITS) ^ ibc_crc_16_tab[(crc >> (CRC_BITS - BYTE_BITS)) ^ *p++];
   return(crc);
 }
...
int	ibc_endian_test_double(void)
 {
   ...
   ed_test.d = 1.2345;
   if (0 == memcmp(&(ed_big.c[0]), &(ed_test.c[0]), 8)) return(IBC_BIG_ENDIAN);
   if (0 == memcmp(&(ed_little.c[0]), &(ed_test.c[0]), 8)) return(IBC_LITTLE_ENDIAN);
...
int	ibc_hton_alert(IBAS_ALERT *a)
 {
   ...
   a->object_type = htonl(a->object_type);
...
int	ibc_ntoh_alert(IBAS_ALERT *a)
 {
   ...
   a->object_type = ntohl(a->object_type);
...
int	ibc_ntoh_dl(IBC_DL *dl)
 {
   ...
   if (ibc_crc_16(0xffff, (void *)dl, sizeof(*dl)))  return(IBC_BAD_CRC);
